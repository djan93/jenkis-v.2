 ...
 stage ("Deploy Application on Azure Kubernetes Service")
 {
  switch (env.BRANCH_NAME) {
    ...
    case "canary":
        ...
        sh("kubectl --namespace=jenkins apply -f k8s/canary/")
        sh("echo http://`kubectl --namespace=jenkins get service/${appName} --output=json | jq -r '.status.loadBalancer.ingress[0].ip'` > ${appName}")
        break
    ...
    case "master":
        ...
        sh("kubectl --namespace=jenkins apply -f k8s/production/")
        sh("echo http://`kubectl --namespace=jenkins get service/${appName} --output=json | jq -r '.status.loadBalancer.ingress[0].ip'` > ${appName}")
        break
